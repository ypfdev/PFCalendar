...
@interface PFCalendarContentView()<UICollectionViewDataSource,UICollectionViewDelegate>{
    //是否是在点击日期或者滑动改变页数
...
@implementation PFCalendarContentView
...
- (void)setEventSource:(id<PFCalendarEventSource>)eventSource{
    ...
    if (![PFCalendarAppearance share].defaultSelected) {
...
- (void)initUI
{
    ...
    self.flowLayout = [PFCalendarCLFlowLayout new];
    self.flowLayout.itemSize = CGSizeMake(self.frame.size.width/7, [PFCalendarAppearance share].weekDayHeight);
    self.flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    self.flowLayout.itemCountPerRow = 7;
//    self.flowLayout.rowCount = [PFCalendarAppearance share].isShowSingleWeek ? 1:[PFCalendarAppearance share].weeksToDisplay;
    self.flowLayout.rowCount = [PFCalendarAppearance share].weeksToDisplay;
    ...
    [self.collectionView registerClass:[PFCalendarCLCell class] forCellWithReuseIdentifier:@"dayCell"];
//    self.collectionView.contentInset = UIEdgeInsetsMake(0, 0,[PFCalendarAppearance share].isShowSingleWeek ? [PFCalendarAppearance share].weekDayHeight*([PFCalendarAppearance share].weeksToDisplay-1) : 0, 0);
    self.backgroundColor = [PFCalendarAppearance share].calendarBgColor;
    
    ///先初始化数据
    [self getDateDatas];
    
    UIView *maskView = [[UIView alloc] initWithFrame:self.bounds];
    maskView.backgroundColor = [PFCalendarAppearance share].calendarBgColor;
    maskView.alpha = 0;
...
- (void)setSingleWeek:(BOOL)singleWeek{
    [PFCalendarAppearance share].isShowSingleWeek = singleWeek;
//    self.flowLayout.rowCount = singleWeek ? 1:[PFCalendarAppearance share].weeksToDisplay;
...
- (CGRect)obtainVisualFrame{
    ...
    return CGRectMake(0 , ([self weekOfMonthWithDate:self.currentDate]-1)*[PFCalendarAppearance share].weekDayHeight, CGRectGetWidth(self.frame), [PFCalendarAppearance share].weekDayHeight);
}
//设置可见的区域
...
//        return  7;
//    }
    return 7*[PFCalendarAppearance share].weeksToDisplay;
}
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    PFCalendarCLCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"dayCell" forIndexPath:indexPath];
    PFCalendarDayItem *item = self.daysInMonth[indexPath.section][indexPath.row];
    if ([PFCalendarAppearance share].isShowSingleWeek) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    PFCalendarCLCell *cell = (PFCalendarCLCell*)[collectionView cellForItemAtIndexPath:indexPath];
    cell.isSelected = true;
    PFCalendarDayItem *itemCurrent;
    PFCalendarDayItem *itemLast;
    NSArray *dataSource;
    if ([PFCalendarAppearance share].isShowSingleWeek) {
    ...
    NSDateComponents *comps = [[PFCalendarAppearance share].calendar components:NSCalendarUnitMonth | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekday fromDate:itemCurrent.date];
    ...
    if (touchMonthIndex == currentMonth || [PFCalendarAppearance share].isShowSingleWeek) {
        PFCalendarCLCell *lastCell = (PFCalendarCLCell*)[collectionView cellForItemAtIndexPath:self.currentSelectedIndexPath];
        lastCell.isSelected =  itemLast.isSelected;
    }
    
    NSInteger index = comps.weekday-[PFCalendarAppearance share].calendar.firstWeekday%7;
    if (index < 0) {
        index += 7;
    }
...
- (void)getDateDatas{
    ...
    if (self.currentDate == nil) {
        self.currentDate = [PFCalendarAppearance share].defaultDate;
...
- (void)updatePageWithNewDate:(BOOL)isNew
{
    ...
    if ([PFCalendarAppearance share].isShowSingleWeek) {
        ...
        NSInteger firstWeekday = [PFCalendarAppearance share].firstWeekday%7;
        NSInteger singWeekDefaultSelectedIndex = [PFCalendarAppearance share].singWeekDefaultSelectedIndex%7;
...
- (NSDate *)getNewCurrentDate{
    ...
    if (currentPage == round(NUMBER_PAGES_LOADED / 2) && [PFCalendarAppearance share].isShowSingleWeek){
...
- (CGFloat)singleWeekOffsetY{
    ...
    return  (comps.weekOfMonth-1)*[PFCalendarAppearance share].weekDayHeight;
}
...
- (NSDate *)beginningOfWeek:(NSDate *)date{
    ...
    NSInteger index = componentsCurrentDate.weekday-[PFCalendarAppearance share].calendar.firstWeekday%7;
    if (index < 0) {
        index += 7;
    }
...
- (NSArray *)getDaysOfWeek:(NSDate *)date
{
    ...
    for (NSInteger i=0; i<7; i++) {
        ...
        PFCalendarDayItem *item = [PFCalendarDayItem new];
        item.isOtherMonth = monthIndex != self.currentMonthIndex;
        item.date = currentDate;
        if ([self isEqual:currentDate other:self.currentDate]) {
            item.isSelected = YES;
           
            if ([PFCalendarAppearance share].defaultSelected || !self.currentSelectedIndexPath) {
            ...
            if ([PFCalendarAppearance share].isShowSingleWeek) {
                ...
                if (beginWeekIndexPath) {
                    ...
                    NSInteger originX = self.frame.origin.y/[PFCalendarAppearance share].weekDayHeight;
                    NSInteger x = [PFCalendarAppearance share].weeksToDisplay-originX-1;
        ...
        item.eventDotColor = [PFCalendarAppearance share].dayDotColor;
...
- (NSArray*)getDaysOfMonth:(NSDate *)date
{
    ...
    for (NSInteger i = 0; i<[PFCalendarAppearance share].weeksToDisplay; i++) {
        NSDateComponents *dayComponent = [NSDateComponents new];
        dayComponent.day = 7;
